...
class FastSpeech2Loss(nn.Module):
    ...
    def forward(self, inputs, predictions, step):
        ...
        for mel_prediction in mel_predictions:
            mel_prediction = mel_prediction.masked_select(mel_masks.unsqueeze(1))
            mel_loss += F.mse_loss(mel_prediction, mel_targets, reduction="sum") # sum_{i=1}^{3}
        mel_loss = mel_loss / (self.n_mel_channels * mel_lens) # sum_{i=1}^{3} / {T*M}
        ...
        if step > self.var_start_steps:
            ...
            pitch_loss = torch.zeros(1).to(mel_loss.device)
            duration_loss = torch.zeros(1).to(mel_loss.device)
        
        total_loss = (
            mel_loss + duration_loss + pitch_loss + ctc_loss + bin_loss
        )
